/* =========================================================
   CLIENTES.JS - v1.1 (Seguro e Funcional)
   - Usa readLS/writeLS do script.js (padrão global)
   - BaseKey: "clientes"  => script.js resolve para acc_${empresaId}__clientes
   - Corrige bug de sintaxe (R perdido)
========================================================= */
...
(function () {
  ...
  function gerarUUID() {
    // UUID v4 simples (suficiente pro front)
  ...
  // ? Fallback seguro se script.js ainda NÃO carregou
  function readSafe(key) {
    if (window.readLS) return window.readLS(key);
    try {
      // fallback: tenta ler direto da chave base (sem prefixo) - melhor que quebrar
  ...
  function writeSafe(key, value) {
    if (window.writeLS) return window.writeLS(key, value);
    try {
  ...
  function aplicarFiltros(clientes) {
    ...
    return (clientes || []).filter((cli) => {
      if (f.busca) {
        const texto = [
          cli.nome, cli.cpfCnpj, cli.tipo, cli.cidade, cli.uf, cli.telefone, cli.email, cli.obs
        ].join(" ").toLowerCase();
  ...
  function renderClientes() {
    ...
    const clientesRaw = readSafe(CHAVE);
    ...
    if (!window.renderTableLazy) {
      // fallback simples
      tbody.innerHTML = clientes.map((c) => `<tr><td>${c.nome || ""}</td></tr>`).join("");
      return;
    }
    ...
    window.renderTableLazy(
      ...
      (c) => {
        ...
        const telefoneNumeros = String(c.telefone || "").replace(/\D/g, "");
  ...
  function salvarCliente() {
    ...
    if (editId) {
      ...
      if (idx > -1) {
        dados.id = editId;
        dados.createdAt = clientes[idx]?.createdAt || dados.updatedAt;
        clientes[idx] = dados;
  ...
  function attachEvents() {
    document.getElementById("abrir-modal-cliente")?.addEventListener("click", () => abrirModal(null));
  ...
  document.addEventListener("DOMContentLoaded", () => {
    console.log("? Clientes.js v1.1 carregado");

    attachEvents();
    renderClientes();

    // quando o sync terminar e atualizar localStorage, re-renderiza
