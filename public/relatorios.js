...
(function () {
  ...
  function renderResumo() {
    ...
    // ??? Média POR DIA - OPÇÃO B (dias com venda concluída no Período)
    const diasComVendaSet = new Set();
    vendasPeriodo.forEach(v => {
      const d = parseBRDate(v.data);
      if (!d) return;
      const key = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
      diasComVendaSet.add(key);
    });
    const diasComVenda = diasComVendaSet.size || 1;
    const media = lucro / diasComVenda;
    ...
    debug(`?? Elementos DOM atualizados:`, {
      ...
      mediaDia: document.getElementById("resumo-media")?.textContent,
      diasComVenda
    });
